...
import { getAppointmentSlots } from 'apiSdk/appointment-slots';
import { AppointmentSlotInterface } from 'interfaces/appointment-slot';

import { AppointmentSlotListPage as ShowCaseList } from 'pages/appointment-slots';
...
function DashboardPage() {
  ...
  const appointmentSlotStat = useSWR<PaginatedInterface<AppointmentSlotInterface>>(
    () => `/appointment-slots`,
    () => getAppointmentSlots({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    },
    {
      ...
      result: appointmentSlotStat?.data?.totalCount,
      isLoading: appointmentSlotStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Appointment Slots
                </Text>
                <NextLink href={`appointment-slots`} passHref legacyBehavior>
                  <Button
