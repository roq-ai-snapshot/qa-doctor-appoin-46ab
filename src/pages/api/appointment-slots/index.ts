...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getAppointmentSlots() {
    ...
    const response = await prisma.appointment_slot
      .withAuthorization({
        roqUserId,
      ...
      .findManyPaginated({
        ...convertQueryToPrismaUtil(query, 'appointment_slot'),
        take: limit,
        skip: offset,
        ...(order?.length && {
  ...
  async function createAppointmentSlot() {
    await appointmentSlotValidationSchema.validate(req.body);
    const body = { ...req.body };

    const data = await prisma.appointment_slot.create({
      data: body,
    });
